...
if %1 == 1 (echo Making RM07 Windows DEMO VERSION distribution ...) else (echo Making RM07 Windows FULL VERSION distribution ...)
...
REM - msbuild RM07-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild RM07.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild RM07.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\RM07.aaxplugin\Contents\Win32\RM07.aaxplugin --out .\build-win\aax\bin\RM07.aaxplugin\Contents\Win32\RM07.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\RM07.aaxplugin\Contents\x64\RM07.aaxplugin --out .\build-win\aax\bin\RM07.aaxplugin\Contents\x64\RM07.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\RM07.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\RM07.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "RM07 Installer" ".\installer\RM07 Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\RM07 Installer.exe" ".\installer\RM07 Demo Installer.exe"
REM -del ".\installer\RM07 Installer.exe"
REM -)
