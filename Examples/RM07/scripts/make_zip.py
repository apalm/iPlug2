...
def  main():
  ...
  installer = "\installer\RM07 Installer.exe"
   
  if demo:
    installer = "\installer\RM07 Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\RM07 manual.pdf" 
  ]
  ...
  ZIPNAME = "RM07-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "RM07-v" + FULLVERSIONSTR + "-win-demo.zip"
